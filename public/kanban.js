...
// // ボードとタスクのデータを保存する配列
// let boards = JSON.parse(localStorage.getItem('boards')) || []; // ローカルストレージからボードデータを取得、ない場合は空配列
// let tasks = JSON.parse(localStorage.getItem('tasks')) || []; // ローカルストレージからタスクデータを取得、ない場合は空配列

// // リクエスト管理用変数 リクエスト回数確認用
// const requestTracker = new Set(); // 送信済みリクエストIDを管理
...
//     title: boardTitle, // トリム後のボード名
//   };

//   //  リクエスト回数確認用  開始
//   const requestID = crypto.randomUUID(); // リクエストごとに一意のIDを生成
//   if (requestTracker.has(requestID)) {
//     console.warn(`Duplicate request detected: ID=${requestID}`);
//     return; // 同じリクエストIDの場合は処理を中断
//   }
//   requestTracker.add(requestID); // リクエストIDを登録
//   boardForm.querySelector('button[type="submit"]').disabled = true; // ボタン無効化
//   //  リクエスト回数確認用  終了
...
//     boards.push({ id: board.id, name: boardTitle });  // `name` を保存
//     saveToLocalStorage();
//     renderBoard({ id: board.id, name: boardTitle });
//     // 入力フィールドをクリア
...
//   column.innerHTML = `
//     <h2 class="board-name-style">${board.name}</h2>
...
//     </div>
//     <div>
//       <div class="memo-title-style">メモ</div>
//       <p class="memo-style">${task.memo}</p>
//     </div>
...
//   const newBoardID = e.target.closest('.kanban-column').id; // 移動先ボードIDを取得

//   e.target.closest('.kanban-column').appendChild(task); // タスクを新しいボードに移動

//   const boardID = e.target.closest('.kanban-column').id; // 新しいボードのIDを取得
//   tasks = tasks.map(t => t.taskID === taskId ? { ...t, boardID } : t); // タスクの所属ボードIDを更新
//   saveToLocalStorage(); // ローカルストレージを更新
...
//     body: JSON.stringify({ boardID: newBoardID }) // 更新するボードIDを送信
...
// function initialize() {
//   if (boards.length === 0) {
//     // 初期ボードが存在しない場合、デフォルトのボードを作成
//     const initialBoards = ['todo', 'in-progress', 'done']; // デフォルトのボード名
//     initialBoards.forEach(boardName => {
//       addBoard(boardName); // 各ボードを追加
//     });
//   }

//   boards.forEach(board => renderBoard(board)); // 既存のボードを描画
//   tasks.forEach(task => {
//     const boardElement = document.getElementById(task.boardID);
//     if (boardElement) {
//       renderTask(task); // 既存のタスクを描画
//     }
//   });
...
//修正後（ サーバーから返却された boardID を適切に使用する）カンバンボード方式のタスク管理アプリ
